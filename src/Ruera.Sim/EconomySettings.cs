...
/// economiche», RUE-43): wage, fines, delivery/training delays and the shift
/// budget. Immutable config like the calendar: not hashed on <see cref="SimState"/>,
/// but part of the game's identity — replay and load must receive the same
/// settings (folds into the scenario hash via <see cref="Scenario.Scenario"/>).
/// </summary>
public sealed record EconomySettings(
    ...
    long ShiftMinutes)
{
    /// <summary>Vertical-slice defaults: the constants hardcoded before RUE-43.</summary>
